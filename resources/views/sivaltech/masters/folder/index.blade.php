       ...
       <div class="box-header with-border">
        ...
        <div class="col-md-12">
          <div class="pull-right">
            ...
            <a href="{{route('vidyaaarambha.masters.folder.create')}}" class="btn bg-green margin" data-toggle="tooltip" data-placement="bottom" title="Click here to add" id="add_asset">
          ...
          @foreach($folder as $sd)
            <tr style="page-break-inside: avoid;" tabindex="-1" data-toggle="popover" data-placement="top" data-trigger="focus" data-html="true" data-content="Created by: {{$sd->created_by}}<br> Created at: {{getFormatedDate($sd->created_at)}} <br> Updated by:{{$sd->updated_by}} <br> Updated at:{{getFormatedDate($sd->updated_at)}}">
              ...
              @endif
                ...
                <td>
                <a href="{{URL::to('/')}}/vidyaaarambha/masters/folder/{{$sd->id}}/edit" class="td-action-btn point-this" data-toggle="tooltip" data-placement="top" title="Edit">
                  <i class="glyphicon glyphicon-edit"></i>
                </a>

              {{--   <a href="{{URL::to('/')}}/vidyaaarambha/masters/folder/{{$sd->id}}" class="td-action-btn point-this" data-toggle="tooltip" data-placement="top" title="View" >
                  <i class="glyphicon glyphicon-eye-open"></i>
                </a> --}}

               @if($sd->deleted_at==null)
                  <a href="{{URL::to('/')}}/vidyaaarambha/masters/folder/deactivate/{{$sd->id}}" class="td-action-btn text-danger" data-toggle="tooltip" data-placement="top" title="" data-original-title="Deactivate" onclick="return confirm_delete('Deactivate'); ">
                    <i class="fa fa-lock text-red" style='font-size:18px;'></i>
                  </a>
                @else
                  <a href="{{URL::to('/')}}/vidyaaarambha/masters/folder/deactivate/{{$sd->id}}" class="td-action-btn text-success" data-toggle="tooltip" data-placement="top" title="" data-original-title="Activate" onclick="return confirm_delete('Activate');">
                    <i class="fa fa-lock text-red" style='font-size:18px;'></i>
                  </a>
                @endif
              </td>
